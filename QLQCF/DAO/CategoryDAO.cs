...
namespace QLQCF.DAO
{
    public class CategoryDAO
    {
        ...
        public bool UpdateCategory(string name, int id,int active)
        {
            string query = string.Format("update dbo.FoodCategory set name = N'{0}',active={2} where ID_FoodCategory = {1}", name, id.ToString(),active.ToString());
            int result = DataProvider.Instance.ExecuteNonQuery(query);
        ...
        public Category GetCateWithID(int id)
        {
            DataTable data = DataProvider.Instance.ExecuteQuery("select * from FoodCategory where ID_FoodCategory='" + id + "' and Active='1'");
            if (data.Rows.Count > 0)
            {
                return new Category(data.Rows[0]);
            }
